...
<script>
    layui.use(['form', 'table'], function () {
        ...
        table.render({
            elem: '#currentTableId',
            url: '${pageContext.request.contextPath}/reader/queryAllReader',
            toolbar: '#toolbarDemo',
            ...
            cols: [[
                ...
                {field: 'birthday', width: 140, title: '出生日期', templet: '<div>{{ layui.util.toDateString(d.birthday, "yyyy年MM月dd日") }}</div>'},
        ...
        table.on('toolbar(currentTableFilter)', function (obj) {
            if (obj.event === 'add') {  // 监听添加操作
                var index = layer.open({
                    ...
                    shadeClose: true,
                    area: ['100%', '100%'],
                    content: '${pageContext.request.contextPath}/reader/readerAdd',
                });
        ...
        function updatePasswordById(id ,index){
            $.ajax({
                url: "${pageContext.request.contextPath}/reader/updatePasswordById",
                type: "POST",
                data: {id: id},
                success: function (data) {
                    if (data.code === 1) {
        ...
        table.on('tool(currentTableFilter)', function (obj) {
            ...
            if (obj.event === 'edit') {
                const index = layer.open({
                    ...
                    shadeClose: true,
                    area: ['100%', '100%'],
                    content: '${pageContext.request.contextPath}/reader/readerEdit?id=' + data.id,
                });
                $(window).on("resize", function () {
                    layer.full(index);
                });
                ...
                layer.confirm('确定要重置读者密码为初始密码吗?', function (index) {
